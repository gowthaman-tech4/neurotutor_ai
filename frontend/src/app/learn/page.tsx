    ...
    return (
        <div className="max-w-3xl mx-auto px-6 py-8">
                ...
                <div className="flex flex-col items-center justify-center min-h-[60vh] text-center">
                    ...
                    <div className="glass-card flex items-center gap-3 px-4 py-3 w-full max-w-lg glow">
                        ...
                        <button
                            ...
                            className="px-5 py-2 rounded-full bg-gradient-to-r from-violet-600 to-purple-600 text-white text-sm font-medium transition-smooth disabled:opacity-40"
                        >
                    ...
                    <div className="flex flex-wrap gap-2 mt-6 justify-center">
                        {["Quadratic Equations", "Newton's Laws", "Photosynthesis", "Trigonometry", "Chemical Bonding"].map((t) => (
                            <button key={t} onClick={() => setTopic(t)}
                                className="px-3 py-1.5 rounded-full bg-white/5 text-gray-400 text-xs hover:bg-white/10 hover:text-white transition-smooth">
                                {t}
                            </button>
                        ))}
                    </div>
            ...
            {phase === "lesson" && steps.length > 0 && (
                <div>
                    ...
                    <div className="flex gap-2 mb-6">
                        {steps.map((_, i) => (
                            <div key={i} className={`h-1.5 flex-1 rounded-full transition-all duration-500 ${i <= currentStep ? "bg-violet-500" : "bg-white/10"}`} />
                        ))}
                    </div>
                    ...
                    <div className="glass-card p-6 mb-4">
                        ...
                        <p className="text-gray-300 text-sm leading-relaxed mb-4">{steps[currentStep].explanation}</p>

                        {steps[currentStep].example && (
                            <div className="bg-violet-500/10 border border-violet-500/20 rounded-xl p-4 mb-4">
                                <div className="text-xs font-semibold text-violet-400 uppercase tracking-wider mb-1">Example</div>
                                <p className="text-sm text-gray-200">{steps[currentStep].example}</p>
                            </div>
                        )}
                        ...
                        {steps[currentStep].visual_hint && steps[currentStep].visual_hint !== "N/A" && (
                            <div className="bg-cyan-500/10 border border-cyan-500/20 rounded-xl p-4 mb-4">
                                <div className="text-xs font-semibold text-cyan-400 uppercase tracking-wider mb-1">🎨 Visual</div>
                                <p className="text-sm text-gray-300">{steps[currentStep].visual_hint}</p>
                            </div>
                        )}
                        ...
                        {steps[currentStep].mini_check && (
                            <div className="bg-amber-500/10 border border-amber-500/20 rounded-xl p-4">
                                ...
                                <p className="text-sm text-gray-300">{steps[currentStep].mini_check}</p>
                            </div>
                        )}
                    </div>
                    ...
                    <div className="flex gap-3 justify-end">
                        ...
                        {currentStep < steps.length - 1 ? (
                            <button onClick={() => setCurrentStep((p) => p + 1)}
                                className="px-5 py-2.5 rounded-full bg-gradient-to-r from-violet-600 to-purple-600 text-white text-sm font-medium transition-smooth">
                                Next →
                            </button>
                        ) : (
                            <button onClick={startQA}
                                className="px-5 py-2.5 rounded-full bg-gradient-to-r from-emerald-600 to-teal-600 text-white text-sm font-medium transition-smooth glow">
            ...
            {phase === "qa" && (
                <div>
                    <div className="flex items-center justify-between mb-6">
                        <h2 className="text-xl font-bold text-white">🧠 Understanding Check: {topic}</h2>
                        <span className="text-xs text-gray-500">Q {qaStep + 1} of {QA_QUESTIONS.length}</span>
                    </div>

                    <div className="flex gap-2 mb-6">
                        {QA_QUESTIONS.map((_, i) => (
                            <div key={i} className={`h-1.5 flex-1 rounded-full transition-all duration-500 ${i <= qaStep ? "bg-cyan-500" : "bg-white/10"}`} />
                        ))}
                    </div>

                    <div className="glass-card p-6 mb-4">
                        <div className="text-xs font-semibold text-cyan-400 uppercase tracking-wider mb-2">Open-Ended Question</div>
                        <p className="text-gray-200 text-sm">{QA_QUESTIONS[qaStep].text}</p>
                    </div>
                        ...
                        <>
                            <div className="glass-card p-4 mb-4">
                                <textarea value={qaAnswer} onChange={(e) => setQaAnswer(e.target.value)}
                                    ...
                                    className="w-full bg-transparent text-white placeholder-gray-500 outline-none text-sm resize-none min-h-[100px]" />
                            </div>
                            <div className="flex gap-3 justify-end">
                                <button onClick={() => startPractice()}
                                    className="px-4 py-2 rounded-full border border-white/10 text-gray-500 text-xs hover:bg-white/5 transition-smooth">
                                    Skip Q&A
                                </button>
                                <button onClick={submitQA} disabled={qaLoading || !qaAnswer.trim()}
                                    className="px-5 py-2.5 rounded-full bg-gradient-to-r from-cyan-600 to-blue-600 text-white text-sm font-medium transition-smooth disabled:opacity-40">
                        ...
                        <>
                            ...
                            <div className="glass-card p-5 mb-4">
                                <div className="flex items-center gap-3 mb-3">
                                    ...
                                    <div>
                                        <div className="text-sm text-white font-medium">Understanding</div>
                                        <div className="text-xs text-gray-500">Style: <span className="text-violet-400">{thoughtResult.detected_style}</span></div>
                                    </div>
                                </div>
                                <p className="text-sm text-gray-300 mb-3">{thoughtResult.understanding_feedback}</p>
                            ...
                            <div className="glass-card p-5 mb-4">
                                <div className="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-3">🧠 Your Thinking Pattern</div>
                                    ...
                                    {Object.entries(thoughtResult.thought_patterns || {}).map(([key, val]) => (
                                        <div key={key}>
                                            ...
                                            <div className="w-full bg-white/5 rounded-full h-1.5">
                                                <div className="h-1.5 rounded-full bg-gradient-to-r from-violet-500 to-cyan-500 transition-all duration-700" style={{ width: `${val}%` }} />
                                            </div>
                                        </div>
                                    ))}
                                </div>
                            ...
                            {thoughtResult.personalization_hints && (
                                <div className="bg-violet-500/10 border border-violet-500/20 rounded-xl p-4 mb-4">
                                    <div className="text-xs font-semibold text-violet-400 uppercase tracking-wider mb-2">✨ How I'll Teach You</div>
                                    <p className="text-sm text-gray-300 mb-1">📌 {thoughtResult.personalization_hints.preferred_examples}</p>
                                    <p className="text-sm text-gray-300 mb-1">📖 {thoughtResult.personalization_hints.explanation_approach}</p>
                                </div>
                            )}
                            ...
                            <div className="flex gap-3 justify-end">
                                <button onClick={nextQAOrPractice}
                                    ...
                                    {qaStep < QA_QUESTIONS.length - 1 ? "Next Question →" : "✏️ Start Practice"}
                                </button>
                            </div>
                        </>
                    )}
            ...
            {(phase === "practice" || phase === "hint") && (
                <div>
                    <div className="flex items-center justify-between mb-6">
                        <h2 className="text-xl font-bold text-white">✏️ Practice: {topic}</h2>
                        <div className="flex items-center gap-3 text-xs text-gray-500">
                            <span>Difficulty: <span className="text-violet-400">{difficulty}</span></span>
                            <span>Hints: <span className="text-amber-400">{hintsUsed}</span></span>
                            <span>Attempts: <span className="text-cyan-400">{attempts}</span></span>
                        </div>
                    </div>

                    <div className="glass-card p-6 mb-4">
                        <div className="text-xs font-semibold text-violet-400 uppercase tracking-wider mb-2">Question</div>
                    ...
                    <div className="glass-card p-4 mb-4">
                        <textarea
                            ...
                            className="w-full bg-transparent text-white placeholder-gray-500 outline-none text-sm resize-none min-h-[80px]"
                        />
                    </div>
                    ...
                    <div className="flex gap-3 justify-end">
                        ...
                        <button onClick={() => setPhase("practice")} disabled={phase !== "hint"}
                            className="px-5 py-2.5 rounded-full border border-white/10 text-gray-400 text-sm hover:bg-white/5 transition-smooth disabled:opacity-40">
                            Try Again
                        </button>
                        <button onClick={submitAnswer} disabled={loading || !answer.trim()}
                            className="px-5 py-2.5 rounded-full bg-gradient-to-r from-violet-600 to-purple-600 text-white text-sm font-medium transition-smooth disabled:opacity-40">
            ...
            {phase === "evaluate" && evalResult && (
                <div>
                    <h2 className="text-xl font-bold text-white mb-6">📝 Results: {topic}</h2>
                    ...
                    <div className="glass-card p-6 mb-4">
                        <div className={`text-2xl font-bold mb-2 ${evalResult.assessment === "Correct" ? "text-emerald-400" :
                            ...
                            {evalResult.assessment === "Correct" ? "✔ Correct!" :
                                evalResult.assessment === "Partially Correct" ? "⚠️ Partially Correct" : "✘ Incorrect"}
                        </div>
                        <p className="text-sm text-gray-300 mb-3">{String(evalResult.feedback || "")}</p>
                        {evalResult.improvement_tip ? (
                            <p className="text-sm text-violet-300">💡 Tip: {String(evalResult.improvement_tip)}</p>
                        ) : null}
                    </div>
                    ...
                    {masteryData && (
                        <div className="glass-card p-6 mb-4">
                            <div className="flex items-center justify-between mb-3">
                                ...
                                <span className="text-sm font-bold text-violet-400">{masteryData.score.toFixed(1)}%</span>
                            </div>
                            <div className="w-full bg-white/5 rounded-full h-3 mb-2 overflow-hidden">
                                <div
                                    className="h-3 rounded-full bg-gradient-to-r from-violet-500 to-purple-500 transition-all duration-1000"
                                    style={{ width: `${masteryData.score}%` }}
                                />
                            </div>
                            <div className="flex justify-between text-xs text-gray-500">
                                <span>{masteryData.prevScore.toFixed(1)}% → {masteryData.score.toFixed(1)}%</span>
                                <span className="capitalize text-violet-400">{masteryData.level}</span>
                            </div>
                        </div>
                    )}
