...
const TASK_MODES: { key: TaskType; label: string; emoji: string; placeholder: string }[] = [
    { key: "teach", label: "Teach", emoji: "📖", placeholder: "Ask me to explain any concept..." },
    { key: "practice", label: "Practice", emoji: "✏️", placeholder: "Request practice questions on a topic..." },
    { key: "hint", label: "Hint", emoji: "💡", placeholder: "Ask for a hint on your current problem..." },
    { key: "evaluate", label: "Evaluate", emoji: "✅", placeholder: "Type your answer for evaluation..." },
];
...
export default function ChatPage() {
    ...
    const sendMessage = async () => {
        ...
        try {
            ...
            setMessages((prev) => [
                ...prev,
                { role: "ai", content: "⚠️ Failed to reach the API. Make sure the backend is running on port 8000." },
            ]);
        } finally {
            setLoading(false);
        }
            ...
            <div ref={scrollRef} className="flex-1 overflow-y-auto py-6 space-y-4">
                ...
                {loading && (
                    <div className="flex items-center gap-3 px-4">
                        <div className="w-8 h-8 rounded-full bg-gradient-to-br from-violet-500 to-purple-700 flex items-center justify-center text-white text-xs font-bold">N</div>
            ...
            <div className="pb-4 space-y-3">
                ...
                <div className="flex items-center justify-center gap-2 flex-wrap">
                    {TASK_MODES.map((mode) => (
                        <button key={mode.key} onClick={() => setActiveMode(mode.key)}
                            className={`px-4 py-2 rounded-full text-sm font-medium transition-smooth ${activeMode === mode.key ? "bg-violet-600 text-white glow" : "bg-white/5 text-gray-400 hover:bg-white/10 hover:text-white"
                                }`}>
                    ...
                    <div className="relative">
                        <button onClick={() => setShowLangMenu(!showLangMenu)}
                            className="px-3 py-2 rounded-full bg-white/5 text-gray-400 hover:bg-white/10 hover:text-white text-sm font-medium transition-smooth flex items-center gap-1.5">
                            🌐 {selectedLang?.label}
                        </button>
                        {showLangMenu && (
                            <div className="absolute bottom-12 right-0 glass-card p-3 rounded-xl min-w-[180px] z-50 shadow-xl">
                                ...
                                {LANGUAGES.map((lang) => (
                                    <button key={lang.code}
                                        ...
                                        className={`w-full text-left px-3 py-1.5 rounded-lg text-sm transition-smooth ${language === lang.code ? "bg-violet-600/30 text-violet-300" : "text-gray-400 hover:bg-white/5 hover:text-white"
                                            }`}>
                                        {lang.label}
                                    </button>
                                ))}
                                    ...
                                    <>
                                        ...
                                        {LANG_MODES.map((m) => (
                                            <button key={m.key}
                                                ...
                                                className={`w-full text-left px-3 py-1.5 rounded-lg text-sm transition-smooth ${langMode === m.key ? "bg-cyan-600/30 text-cyan-300" : "text-gray-400 hover:bg-white/5 hover:text-white"
                                                    }`}>
                                                {m.label}
                                            </button>
                                        ))}
                ...
                <div className="glass-card flex items-center gap-3 px-4 py-3 glow">
                    ...
                    <button onClick={isListening ? stopListening : startListening}
                        className={`w-9 h-9 rounded-full flex items-center justify-center text-sm transition-smooth shrink-0 ${isListening ? "bg-red-500 text-white animate-pulse" : "bg-white/5 text-gray-400 hover:bg-white/10 hover:text-white"
                            }`}
                    ...
                    <button onClick={sendMessage} disabled={loading || !input.trim()}
                        className="px-5 py-2 rounded-full bg-gradient-to-r from-violet-600 to-purple-600 text-white text-sm font-medium hover:from-violet-500 hover:to-purple-500 transition-smooth disabled:opacity-40 disabled:cursor-not-allowed">
                        Send
                    </button>
                </div>
            </div>
...
function ChatBubble({ message, index, speakingIdx, onSpeak }: {
    ...
    return (
        <div className={`flex items-start gap-3 px-2 ${isUser ? "flex-row-reverse" : ""}`}>
            <div className={`w-8 h-8 rounded-full flex items-center justify-center text-white text-xs font-bold shrink-0 ${isUser ? "bg-gradient-to-br from-cyan-500 to-blue-600" : "bg-gradient-to-br from-violet-500 to-purple-700"
                }`}>
                {isUser ? "Y" : "N"}
            </div>
            <div className={`max-w-[75%] rounded-2xl text-sm leading-relaxed ${isUser
                    ? "bg-gradient-to-r from-cyan-600/20 to-blue-600/20 border border-cyan-500/20 text-gray-100 px-4 py-3"
                    : "glass-card text-gray-200 px-4 py-3"
                }`}>
                {!isUser && message.engine && (
                    <div className="text-xs text-violet-400 font-semibold mb-1.5 uppercase tracking-wider">
