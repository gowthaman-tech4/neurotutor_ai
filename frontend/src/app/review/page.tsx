    ...
    <div className="max-w-3xl mx-auto px-6 py-8">
        ...
        <div>
          <div className="text-center mb-6">
            ...
            <p className="text-gray-600 text-sm">
              ...
              {revision > 1 && <span className="text-emerald-400 ml-2">Draft {revision}</span>}
            </p>
          </div>
          ...
          <div className="flex gap-2 justify-center mb-4">
            {REVIEW_MODES.map((m) => (
              <button key={m.key} onClick={() => setReviewMode(m.key)}
                className={`px-4 py-2 rounded-full text-sm font-medium transition-smooth ${reviewMode === m.key ? "bg-emerald-600 text-gray-900 glow" : "bg-black/5 text-gray-600 hover:bg-black/10 hover:text-gray-900"
                  }`}>
                {m.emoji} {m.label}
              </button>
            ))}
            ...
            <div className="flex gap-2 justify-center mb-4">
              {SUBMISSION_TYPES.map((t) => (
                <button key={t.key} onClick={() => setSubType(t.key)}
                  className={`px-3 py-1.5 rounded-full text-xs font-medium transition-smooth ${subType === t.key ? "bg-teal-600/30 text-teal-300" : "bg-black/5 text-gray-500 hover:bg-black/10"
                    }`}>
                  {t.emoji} {t.label}
                </button>
              ))}
          ...
          <div className="glass-card p-4 mb-4">
            <textarea value={content} onChange={(e) => setContent(e.target.value)}
              ...
              className="w-full bg-transparent text-gray-900 placeholder-gray-500 outline-none text-sm resize-none min-h-[200px] font-mono leading-relaxed" />
          </div>

          <div className="flex gap-3 justify-end">
            <button onClick={submit} disabled={!content.trim() || loading}
              className="px-6 py-2.5 rounded-full bg-gradient-to-r from-emerald-600 to-green-600 text-gray-900 text-sm font-medium transition-smooth disabled:opacity-40 glow">
      ...
      {phase === "analyzing" && (
        <div className="flex flex-col items-center justify-center min-h-[50vh]">
          ...
          <div className="w-64 bg-black/5 rounded-full h-2 overflow-hidden">
            <div className="h-2 bg-gradient-to-r from-emerald-500 to-green-500 rounded-full animate-pulse" style={{ width: "75%" }} />
          </div>
        </div>
      )}
          ...
          {reviewMode === "originality" && origResult && (
            <div>
              <div className="glass-card p-6 mb-6 text-center">
                ...
                <div className="w-full bg-black/5 rounded-full h-2.5 mt-3 mb-3 overflow-hidden">
                ...
                <div className={`mt-3 text-sm font-medium ${origResult.is_safe_to_submit ? "text-emerald-400" : "text-red-400"}`}>
                  {origResult.is_safe_to_submit ? " Safe to submit" : " Needs revision"}
                </div>
              </div>

              {origResult.overall_assessment && (
                <div className="glass-card p-4 mb-4">
                  <p className="text-sm text-gray-700">{origResult.overall_assessment}</p>
                </div>
              )}
              ...
              {origResult.flagged_sections && origResult.flagged_sections.length > 0 && (
                <div className="space-y-2 mb-4">
                  <h3 className="text-xs font-semibold text-gray-600 uppercase tracking-wider mb-2"> Flagged Sections</h3>
                    ...
                    return (
                      <div key={i} className={`rounded-xl border ${sevColor} overflow-hidden`}>
                        <button onClick={() => setExpandedItem(isExp ? null : key)}
                          className="w-full text-left p-4 hover:bg-black/5 transition-smooth">
                          <div className="flex items-start gap-2">
                            ...
                            <p className="text-sm text-gray-700 italic flex-1">&ldquo;{f.text}&rdquo;</p>
                          </div>
                        </button>
                        {isExp && (
                          <div className="px-4 pb-4 space-y-2 border-t border-black/5 pt-3">
                            ...
                            <div className="bg-emerald-500/10 border border-emerald-500/20 rounded-lg p-3">
                              <div className="text-xs font-semibold text-emerald-400 mb-1">💡 Rewrite Guidance</div>
                              <p className="text-sm text-gray-700">{f.rewrite_hint}</p>
                            </div>
                          </div>
                        )}
                      </div>
                ...
                <div className="glass-card p-4 mb-4">
                  ...
                  {origResult.uncited_claims.map((c, i) => (
                    <div key={i} className="text-sm text-gray-700 mb-2">
                      <span className="text-gray-600">•</span> &ldquo;{c.claim}&rdquo; — <span className="text-gray-500">{c.reason}</span>
                    </div>
                  ))}
                </div>
              )}
          ...
          {reviewMode === "citations" && citResult && (
            <div>
              <div className="glass-card p-6 mb-6 text-center">
                ...
                <div className="text-4xl font-bold text-emerald-400">{citResult.total_claims_found}</div>
              ...
              <div className="space-y-3 mb-4">
                {citResult.citations_needed?.map((c, i) => (
                  <div key={i} className="glass-card p-4">
                    <p className="text-sm text-gray-700 mb-2">&ldquo;{c.claim_text}&rdquo;</p>
                    ...
                    <div className="bg-emerald-500/10 border border-emerald-500/20 rounded-lg p-3">
                      ...
                      <p className="text-sm text-gray-800 font-mono">{c.suggested_citation}</p>
                    </div>
                  </div>
                ))}
              </div>
              ...
              {citResult.citation_tips && citResult.citation_tips.length > 0 && (
                <div className="bg-emerald-500/10 border border-emerald-500/20 rounded-xl p-4 mb-4">
                  <div className="text-xs font-semibold text-emerald-400 uppercase tracking-wider mb-2">💡 Citation Tips</div>
          ...
          {reviewMode === "feedback" && feedbackResult && (
            <div>
              <div className="glass-card p-6 mb-6 text-center">
                ...
                <div className="w-full bg-black/5 rounded-full h-2.5 mt-3 overflow-hidden">
              ...
              {feedbackResult.top_priority_fix && (
                <div className="bg-amber-500/10 border border-amber-500/20 rounded-xl p-4 mb-4">
                  ...
                  <p className="text-sm text-gray-700">{feedbackResult.top_priority_fix}</p>
                </div>
              )}
                  ...
                  return (
                    <div key={cs.name} className="glass-card overflow-hidden">
                      <button onClick={() => setExpandedItem(isExp ? null : cs.name)}
                        className="w-full flex items-center justify-between p-4 hover:bg-black/5 transition-smooth text-left">
                        <div className="flex items-center gap-3">
                          <div className={`text-lg font-bold ${pct >= 80 ? "text-emerald-400" : pct >= 60 ? "text-amber-400" : "text-red-400"}`}>
                            {cs.score}/{cs.max_score}
                          </div>
                          <span className="text-sm text-gray-900 font-medium">{cs.name}</span>
                        </div>
                      ...
                      {isExp && (
                        <div className="px-4 pb-4 space-y-3 border-t border-black/5 pt-3">
                          <p className="text-sm text-gray-700">{cs.explanation}</p>
                          {cs.weakness_quote && (
                            <div className="bg-red-500/10 border border-red-500/20 rounded-lg p-3">
                              <div className="text-xs font-semibold text-red-400 mb-1"> Weak point</div>
                              <p className="text-sm text-gray-600 italic">&ldquo;{cs.weakness_quote}&rdquo;</p>
                            </div>
                          )}
                          <div className="bg-emerald-500/10 border border-emerald-500/20 rounded-lg p-3">
                            <div className="text-xs font-semibold text-emerald-400 mb-1">💡 Suggestion</div>
                            <p className="text-sm text-gray-700">{cs.suggestion}</p>
                          </div>
                          {cs.improved_example && (
                            <div className="bg-emerald-500/10 border border-emerald-500/20 rounded-lg p-3">
                              <div className="text-xs font-semibold text-emerald-400 mb-1"> Improved</div>
                              <p className="text-sm text-gray-800">{cs.improved_example}</p>
                            </div>
                          )}
                        </div>
                      )}
              ...
              {feedbackResult.overall_feedback && (
                <div className="glass-card p-4 mb-4">
                  <div className="text-xs font-semibold text-gray-600 uppercase tracking-wider mb-2">Overall Feedback</div>
                  <p className="text-sm text-gray-700">{feedbackResult.overall_feedback}</p>
                </div>
              )}
            </div>
          )}
