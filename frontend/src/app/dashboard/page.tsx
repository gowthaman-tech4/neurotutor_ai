...
const TIER_STYLES: Record<string, { color: string; bg: string; label: string; progress: number }> = {
    ...
    learning: { color: "text-blue-400", bg: "bg-blue-400/15", label: "Learning", progress: 35 },
    ...
    mastered: { color: "text-purple-400", bg: "bg-purple-400/15", label: "Mastered", progress: 100 },
};

const EXAM_OPTIONS = [
    { key: "school", label: "🎒 School", short: "School" },
    { key: "jee", label: "⚙️ JEE", short: "JEE" },
        ...
        <div className="max-w-5xl mx-auto px-6 py-8">
            ...
            <div className="flex items-center gap-2 mb-6 flex-wrap">
                ...
                {EXAM_OPTIONS.map((e) => (
                    <button key={e.key} onClick={() => setExamMode(e.key)}
                        className={`px-3 py-1.5 rounded-full text-xs font-medium transition-smooth ${examMode === e.key ? "bg-violet-600 text-white" : "bg-white/5 text-gray-400 hover:bg-white/10 hover:text-white"
                            }`}>
                        {e.label}
                    </button>
                ))}
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
                ...
                <div className="glass-card p-6 col-span-1">
                    <h3 className="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-3">Mastery Level</h3>
                    ...
                    <div className="w-full bg-white/5 rounded-full h-2.5 mb-2">
                        <div className="h-2.5 rounded-full transition-all duration-700 bg-gradient-to-r from-violet-500 to-purple-500"
                ...
                <div className="glass-card p-6 col-span-1">
                    <h3 className="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-3">Cognitive Profile</h3>
                ...
                {smartPlan?.readiness ? (
                    <div className="glass-card p-6 col-span-1">
                        <h3 className="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-3">Exam Readiness</h3>
                        <div className={`text-3xl font-bold ${confColor}`}>{smartPlan.readiness.current_score}%</div>
                        <div className="w-full bg-white/5 rounded-full h-2 mt-2 mb-2 overflow-hidden">
                            <div className="h-2 rounded-full bg-gradient-to-r from-violet-500 to-purple-500 transition-all duration-1000"
                                style={{ width: `${smartPlan.readiness.current_score}%` }} />
                        </div>
                        <div className="text-xs text-gray-500 space-y-1">
                            <div className="flex justify-between">
                                <span>Projected</span>
                                <span className="text-violet-400">{smartPlan.readiness.projected_score}%</span>
                            </div>
                            <div className="flex justify-between">
                                <span>Days left</span>
                                <span className="text-white">{smartPlan.days_until_exam}</span>
                            </div>
                            <div className="flex justify-between">
            ...
            {smartPlan?.topic_analysis && smartPlan.topic_analysis.length > 0 && (
                <div className="mb-8">
                    <h3 className="text-sm font-semibold text-gray-400 uppercase tracking-wider mb-3">📈 Topic Analysis</h3>
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                        {smartPlan.topic_analysis.map((t, i) => (
                            <div key={i} className="glass-card p-4">
                                <div className="flex items-center justify-between mb-2">
                                    <span className="text-sm text-white font-medium">{t.topic_name}</span>
                                    <div className="flex items-center gap-3 text-xs">
                                        ...
                                        <span className="text-gray-500">Risk: <span className={t.forgetting_risk > 0.5 ? "text-red-400" : "text-gray-400"}>{(t.forgetting_risk * 100).toFixed(0)}%</span></span>
                                    </div>
                                </div>
            ...
            <div className="glass-card p-6">
                <div className="flex items-center justify-between mb-4">
                    <div>
                        <h2 className="text-lg font-bold text-white">📅 Study Plan</h2>
                    ...
                    <div className="flex items-center gap-2">
                        <button onClick={() => setShowExamSetup(!showExamSetup)}
                            className="px-3 py-2 rounded-full bg-white/5 text-gray-400 text-xs hover:bg-white/10 hover:text-white transition-smooth">
                            ⚙️ {examDate ? "Edit Exam" : "Set Exam"}
                        </button>
                        <button onClick={generatePlan} disabled={planLoading}
                            className="px-5 py-2 rounded-full bg-gradient-to-r from-violet-600 to-purple-600 text-white text-sm font-medium hover:from-violet-500 hover:to-purple-500 transition-smooth disabled:opacity-50">
                ...
                {showExamSetup && (
                    <div className="bg-white/5 rounded-xl p-4 mb-4 grid grid-cols-2 gap-3">
                        <div>
                            <label className="block text-xs text-gray-400 mb-1">Exam Date</label>
                            <input type="date" value={examDate} onChange={(e) => setExamDate(e.target.value)}
                                className="w-full bg-white/5 border border-white/10 rounded-lg px-3 py-2 text-sm text-white outline-none focus:border-violet-500/50 transition-smooth [color-scheme:dark]" />
                        </div>
                        <div>
                            <label className="block text-xs text-gray-400 mb-1">Study hrs/day</label>
                            <input type="number" step="0.5" min="0.5" max="8" value={studyHours} onChange={(e) => setStudyHours(e.target.value)}
                                className="w-full bg-white/5 border border-white/10 rounded-lg px-3 py-2 text-sm text-white outline-none focus:border-violet-500/50 transition-smooth" />
                        </div>
                    </div>
                )}
                        ...
                        <div className="flex gap-2 mb-3 overflow-x-auto pb-1">
                            {smartPlan.daily_plans.map((day, i) => (
                                <button key={i} onClick={() => setActiveDay(i)}
                                    className={`px-3 py-1.5 rounded-full text-xs font-medium whitespace-nowrap transition-smooth ${activeDay === i ? "bg-violet-600 text-white" : "bg-white/5 text-gray-400 hover:bg-white/10"
                        ...
                        {smartPlan.daily_plans[activeDay] && (
                            <div className="space-y-2">
                                ...
                                {smartPlan.daily_plans[activeDay].activities.map((act, j) => (
                                    <div key={j} className="flex items-center gap-3 bg-white/5 rounded-lg p-3">
                                        <div className={`w-2 h-2 rounded-full shrink-0 ${act.activity === "Learn" ? "bg-cyan-400" :
                                            act.activity === "Revise" ? "bg-amber-400" :
                                                act.activity.includes("Quiz") ? "bg-violet-400" : "bg-red-400"
                                            }`} />
                                        <div className="flex-1">
                                            <span className="text-sm text-white">{act.topic}</span>
                ...
                {plan?.error_fallback && (
                    <div className="text-sm text-amber-400 bg-amber-500/10 border border-amber-500/20 rounded-xl p-4 mb-4">
                        ⚠️ {plan.error_fallback}
                    </div>
                )}

                {plan?.strategic_advice && (
                    <div className="text-sm text-violet-300 bg-violet-500/10 border border-violet-500/20 rounded-xl p-4 mb-4">
                ...
                {plan?.daily_schedule && plan.daily_schedule.length > 0 && (
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3">
                        {plan.daily_schedule.map((day) => (
                            <div key={day.day} className="bg-white/5 rounded-xl p-4 border border-white/5 hover:border-violet-500/30 transition-smooth">
                                <div className="text-xs font-bold text-violet-400 mb-2">Day {day.day}</div>
